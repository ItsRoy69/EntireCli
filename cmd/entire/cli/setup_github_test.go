...
// Bootstrap init deliberately stops before identity recovery. The enable
// command validates/selects an agent first, then performs identity recovery,
// and only then allows setup and the initial commit to proceed.
func TestBootstrapInit_DefersGitIdentity(t *testing.T) {
	...
	state, err := runGitHubBootstrapInitWith(context.Background(), io.Discard, io.Discard, GitHubBootstrapOptions{
		InitRepo:             true,
		NoGitHub:             true,
