...
| B5 | checkpoint_remote inherited-setting detection over HTTPS server: a mismatched owner on **either** origin or the *push destination* → fallback used (covers both fork topologies: fork-clone, and clone-upstream-plus-fork where origin matches but the push destination does not — the latter pinned end-to-end by `TestCheckpointSyncRemote_InheritedCheckpointRemoteDoesNotBypassGate`); the case that must NOT fall back is a checkpoint_remote set in `settings.local.json` (today unit-only; run through a real push) | integration (HTTPS) | gb (gr: N/A until checkpoint_remote applies to refs — verify and pin) |
...
- **D-3 forge map / provider table**: `checkpoint_remote` now supports github and gitlab end to end over direct git transport (SSH/HTTPS) — `entire configure --checkpoint-remote gitlab:owner/repo` (see `GITLAB-PROVIDER-GAP.md` / `docs/superpowers/specs/2026-09-17-gitlab-checkpoint-remote-design.md`). Still open: `gitremote.hostToForge` only maps `github.com`→`gh`, so the `entire://` push-through mirror, trails API, repo protection, and issue-linking remain github-only, and GHE/self-hosted gitlab silently degrade (`{repo_id}`, trails). Decide config story before writing tests beyond pinning.
